...
when false:
  proc copy*(st: Statement): Statement =
    template map[T](s: T): T =
      s
    template map(s: Statement): Statement =
      copy s
    template map[T, U](s: (T, U)): untyped =
      (map s[0], map s[1])
    template map(s: seq): seq =
      var arr = newSeq[typeof map s[0]](s.len)
      for i in 0 ..< arr.len:
        arr[i] = map s[i]
      arr
    result = case st.kind
    of skNone: Statement(kind: skNone)
    of skConstant: Statement(kind: skConstant, constant: st.constant)
    of skFunctionCall:
      Statement(kind: skFunctionCall, callee: map st.callee,
        arguments: map st.arguments)
    of skDispatch:
      Statement(kind: skDispatch,
        dispatchees: map st.dispatchees,
        dispatchArguments: map st.dispatchArguments)
    of skSequence: Statement(kind: skSequence, sequence: map st.sequence)
    of skVariableGet:
      Statement(kind: skVariableGet, variableGetIndex: st.variableGetIndex)
    of skVariableSet:
      Statement(kind: skVariableSet, variableSetIndex: st.variableSetIndex,
    ...
    of skIf:
      Statement(kind: skIf, ifCond: map st.ifCond,
        ifTrue: map st.ifTrue, ifFalse: map st.ifFalse)
    of skWhile:
      Statement(kind: skWhile, whileCond: map st.whileCond,
        whileBody: map st.whileBody)
    of skDoUntil:
      Statement(kind: skDoUntil, doUntilCond: map st.doUntilCond,
        doUntilBody: map st.doUntilBody)
    of skEmitEffect:
      Statement(kind: skEmitEffect, effect: map st.effect)
    of skHandleEffect:
      Statement(kind: skHandleEffect, effectHandler: map st.effectHandler,
        effectBody: map st.effectBody)
    of skTuple:
      Statement(kind: skTuple, elements: map st.elements)
    of skList:
      Statement(kind: skList, elements: map st.elements)
    of skSet:
      Statement(kind: skSet, elements: map st.elements)
    of skTable:
      Statement(kind: skTable, entries: map st.entries)
    of skGetIndex:
      Statement(kind: skGetIndex, getIndexAddress: map st.getIndexAddress,
        getIndex: st.getIndex)
    of skSetIndex:
      Statement(kind: skSetIndex, setIndexAddress: map st.setIndexAddress,
        setIndex: st.setIndex,
        setIndexValue: map st.setIndexValue)
    of skUnaryInstruction:
      Statement(kind: skUnaryInstruction, unaryInstructionKind: st.unaryInstructionKind, unary: map st.unary)
    of skBinaryInstruction:
      Statement(kind: skBinaryInstruction, binaryInstructionKind: st.binaryInstructionKind, binary1: map st.binary1, binary2: map st.binary2)
